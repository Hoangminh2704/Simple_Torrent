...
class PeersManager(Thread):
    ...
    def run(self):
        while self.is_active:
            ...
            tracker_peers = self.torrent.tracker.get_peers_from_trackers()

            # Thêm peer mới (nếu có)
            self.add_peers(tracker_peers)

            # Quản lý kết nối hiện tại
            read = [peer.socket for peer in self.peers]
            read_list, _, _ = select.select(read, [], [], 1)
    ...
    def add_peers(self, peers):
        for peer in peers:
            if self._do_handshake(peer):
                self.peers.append(peer)
                logging.info(f"Peer {peer.ip}:{peer.port} added successfully")
            else:
                logging.warning(f"Failed to handshake with peer {peer.ip}:{peer.port}")
    ...
    def log_peers(self):
        logging.info("Current peers:")
        for peer in self.peers:
            logging.info(f"Peer {peer.ip}:{peer.port} - Unchoked: {peer.is_unchoked()}")
